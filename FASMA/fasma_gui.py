...
def synth(args):
    ...
    fout += '%s %s %s %s %s %s %s ' % (
        ...
        args.surfacegravity,
        args.FeH,
        args.microturbulence,
        args.macroturbulence,
        args.vsini,
    )

    fout += 'model:%s,step_wave:%s,step_flux:%s,inter_file:%s,limb:%s,damping:%s' % (
        args.model,
        args.step_wave,
        args.step_flux,
        inter_file,
    ...
    import FASMA
    driver = FASMA.synthMethod(cfgfile='StarMe_synth.cfg', overwrite=None)
    driver.synthdriver()
...
def main():
    ...
    parent_parser.add_argument('--model', help='Grid of models', default='apogee_kurucz', choices=['apogee_kurucz', 'marcs'], metavar='Model atmosphere')
    ...
    abund_parser = subparsers.add_parser(
        'Abundances',
        parents=[parent_parser],
        help='Spectal synthesis for chemical abundance',
    )
